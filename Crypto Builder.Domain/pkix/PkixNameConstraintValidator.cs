...
namespace CryptoBuilder.Pkix
{
    public class PkixNameConstraintValidator
    {
        ...
        private ISet UnionIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2)
        {
            ...
            if (CryptoBuilder.Utilities.Arrays.AreEqual(ipWithSubmask1, ipWithSubmask2))
            {
            ...
            byte[] ipWithSubmask2)
    {
        ...
        private bool IsIPConstrained(byte[] ip, byte[] constraint)
        {
            ...
            return CryptoBuilder.Utilities.Arrays.AreEqual(permittedSubnetAddress, ipSubnetAddress);
        }
        ...
        {
            if (CryptoBuilder.Utilities.Arrays.AreEqual(ip1, ip2))
            {
                return 0;
            }
            if (CryptoBuilder.Utilities.Arrays.AreEqual(Max(ip1, ip2), ip1))
            {
                return 1;
            }
            return -1;
        ...
        private int HashCollection(ICollection coll)
        {
            ...
            while (it1.MoveNext())
            {
                ...
                if (o is byte[])
                {
                    hash += CryptoBuilder.Utilities.Arrays.GetHashCode((byte[])o);
                }
                else
                {
                    hash += o.GetHashCode();
        ...
        private bool SpecialEquals(Object o1, Object o2)
        {
            ...
            {
                return CryptoBuilder.Utilities.Arrays.AreEqual((byte[])o1, (byte[])o2);
            }
            else
            {
                return o1.Equals(o2);
